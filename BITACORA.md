...
> Documento generado automáticamente el 15 de septiembre de 2026 a las 11:37 p. m. por `npm run informe`. No editar a mano: se regenera desde el código para mantenerse al día.
...
| `src/components/DashboardGerencial.tsx` | DashboardGerencial | 597 | Dashboard gerencial: KPIs (totales, promedio de llenado, críticos >80%, rutas ejecutadas), filtros por zona y rango de fechas, gráfico de barras de estado por zona y tabla de contenedores críticos con acción Atender/Vaciar. |
...
| `src/lib/contenedoresStore.ts` | 721 | Almacén de contenedores: carga y semilla desde Supabase, normalización de ubicación (objeto, GeoJSON, EWKT o WKB/EWKB hexadecimal con parseFloat), datos de respaldo en localStorage y registro, vaciado (0% y estado vacio), edición y eliminación en tiempo real. |
