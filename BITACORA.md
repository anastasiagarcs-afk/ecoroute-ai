...
> Documento generado automáticamente el 16 de septiembre de 2026 a las 04:11 p. m. por `npm run informe`. No editar a mano: se regenera desde el código para mantenerse al día.
...
| `src/components/DashboardGerencial.tsx` | DashboardGerencial | 592 | Dashboard gerencial: KPIs (totales, promedio de llenado, críticos >80%, rutas ejecutadas), filtros por zona y rango de fechas, gráfico de barras de estado por zona y tabla de contenedores críticos con acción Atender/Vaciar. |
...
| `src/components/HistorialOperador.tsx` | HistorialOperador | 99 | — |
...
| `src/components/NavRol.tsx` | NavRol | 353 | — |
| `src/components/NotificacionesOperador.tsx` | NotificacionesOperador | 193 | — |
...
| `src/components/PanelRutaOperador.tsx` | PanelRutaOperador | 270 | — |
...
| `src/components/RoutePanel.tsx` | RoutePanel | 829 | Panel de optimización de rutas: selector de contenedores, generación de ruta vía OSRM, persistencia en el historial, gestión de contenedores (vaciar, editar, eliminar) y reinicio a línea recta. |
...
| `src/lib/historialRutas.ts` | 395 | Almacén de historial de rutas con persistencia en Supabase (tabla HistorialRutas), reintentos y respaldo en localStorage. |
...
| `src/lib/operadoresService.ts` | 241 | — |
...
| `src/lib/rolesAutorizados.ts` | 73 | — |
